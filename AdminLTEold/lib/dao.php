...
{    
    ...
    function sum_data($field='' ,$table ,$where='')
    {
     ...
     //Count diff of  Table field
    function diff_data($field,$table,$where='')
    {
        if($where != '')
        {
            $where= 'where ' .$where;
        ...
        $diff_data = mysqli_query($this->conn,"SELECT SUM($table.$field='')-SUM($table.$field='') from `$table` $where" ) or die(mysqli_error($this->conn));
        return $diff_data;
